...
fn handle_tag(tags: &BTreeMap<String, String>, tag_name: &str, tag_value: String) -> Option<PatchOperation> {
    match tags.get(tag_name) {
        ...
        Some(value) => {
            ...
            Some(replace_patch(format!("/spec/tags/{}", tag_name).into(), Value::String(tag_value)))
        }
        None => {
            info!("Adding {} tag: {}", tag_name, tag_value);
            Some(add_patch(format!("/spec/tags/{}", tag_name).into(), Value::String(tag_value)))
        }
    }
}
...
mod tests {
    ...
    fn make_comparable_set(patches: &mut Vec<PatchOperation>) -> BTreeSet<(&str, String, String)> {
        patches.clone().into_iter().map(|p| {
            match p {
                PatchOperation::Add(add) => { ("add", add.path, add.value.as_str().unwrap().to_string()) }
                PatchOperation::Replace(replace) => { ("replace", replace.path, replace.value.as_str().unwrap().to_string()) }
